...
layoutParagraph _ _ tuple Nil acc = (tuple, acc)
layoutParagraph wdt just (lftPr, rgtPr, y) ls acc = layoutParagraph
		...
		(L.drop (length (inlines' ++ outLfts ++ outRgts)) ls)
	...
	inlines' = if null inlines && y >= snd lftPr && y >= snd rgtPr && let (_, float) = L.head ls in float == Inline then [(0, y, L.head ls)] else inlines
...
paint layoutData wnd dc rt = do
	lay <- readIORef layoutData
	font <- createNiceFont
	oldFont <- selectFont dc font
	pos <- liftM nPos (getScrollInfo wnd)
	mapM_ (\(x, y, (z, _)) -> when (overlapping rt (x, y - pos, x + width z, y - pos + height z)) $ draw dc (x, y - pos) z) lay
...
attachLayout :: (Show t, Eq t, Displayable t) => IORef [Paragraph t] -> HWND -> IO (IO ())
attachLayout ref wnd = do
	...
	let onChange = do
		...
		si <- getScrollInfo wnd
		let toDraw = map snd (dropWhile (uncurry (==)) $ zip lay0 lay) ++ drop (length lay0) lay
		withRECT (0, minimum (32767 : map (\(_, y, _) -> y - nPos si) toDraw), 32767, ht) $ \p -> invalidateRect (Just wnd) (Just p) True
	subclassProc wnd $ \proc wnd msg wParam lParam -> do
		if msg == wM_SIZE then
				...
				return ()
		buffered wnd msg $ paint layoutData wnd
