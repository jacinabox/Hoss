...
sizeTable ob = withWindows ob $ \wnd x y wid ht -> moveWindow wnd (fromIntegral x) (fromIntegral $ x + wid) (fromIntegral y) (fromIntegral $ y + ht) True
...
realizeTable text insetWnd ob inTableProc = case obj ob of
	Right (Left tbl) -> do
		...
		res <- mapM (\(col, (wid, ls)) -> liftM ((,) wid) $ mapM (\(row, (_, para)) -> do
					inner <- createWindow name "" (wS_VISIBLE .|. wS_CHILD) (Just 10) (Just 10) (Just 10) (Just 10) (Just outer) Nothing hdl (inTableProc col row select)
					ref <- newIORef para
		...
		writeIORef text $ alter (position ob) (const $ Right (Left (Table outer res))) textVal
		sizeTable ob
	_ -> return ()
