...
data Table = Table {
	wnd :: HWND,
	contents :: [(Int32, [((HWND, IORef [Paragraph Object], IO ()), [Paragraph Object])])] }
...
instance Binary Table where
	put (Table _ contents) = put (map (map snd . snd) contents)
	get = do
		ls <- get
		return (Table undefined (map (second $ map ((,) undefined)) ls))
...
instance Displayable Object where
	...
	draw dc (x, y) obj@(Object _ selected (Right (Left (Table wnd contents))) _ _ _) = do
		dc <- getDC (Just wnd)
...
getIndex wnd = do
	...
	liftM fromIntegral (getWindowLong parent gWLP_USERDATA)

padding = 5

withWindows ob f = do
	let Right (Left tbl) = obj ob
	foldM_ (\y (wid, row) -> do
		let ht = maximum (10 : map (measureCell wid) row)
		foldM_ (\x ((wnd, _, _), _) -> do
			f wnd x y wid ht
...
sizeTable ob = do
	let Right (Left tbl) = obj ob
	(lf, tp, _, _) <- getWindowRect (wnd tbl)
	withWindows ob $ \wnd x y wid ht -> moveWindow wnd (fromIntegral (lf + x)) (fromIntegral (tp + y)) (fromIntegral (lf + x + wid)) (fromIntegral (tp + y + ht)) True

realizeTable text insetWnd ob inTableProc = case obj ob of
	Right (Left tbl) -> do
		hdl <- getModuleHandle Nothing
		let name = mkClassName "Frame"
		outer <- createWindow name "" (wS_VISIBLE .|. wS_CHILD) (Just 10) (Just 10) (Just 10) (Just 10) (Just insetWnd) Nothing hdl (defWindowProc . Just)
		setWindowLong outer gWLP_USERDATA (fromIntegral (position ob))
		select <- newIORef (False, 0, 0)
		res <- mapM (\(col, (wid, ls)) -> liftM ((,) wid) $ mapM (\(row, (_, para)) -> do
					inner <- createWindow name "" (wS_VISIBLE .|. wS_CHILD) (Just 10) (Just 10) (Just 10) (Just 10) (Just insetWnd) Nothing hdl (inTableProc col row select)
					ref <- newIORef para
					onChange <- attachLayout ref inner
					return ((inner, ref, onChange), para))
				(zip [0..] ls))
			(zip [0..] (contents tbl))
		textVal <- readIORef text
		let ob' = ob { obj = Right (Left (Table outer res)) }
		writeIORef text $ alter (position ob) (const $ obj ob') textVal
		sizeTable ob'
	_ -> return ()
