...
initControls wnd = do
	...
	createEditWindow "Times New Roman" (wS_VISIBLE .|. wS_CHILD .|. wS_TABSTOP) 0 (Just 10) (Just 10) (Just 200) (Just 20) dialogCtrl Nothing hdl
	createEditWindow "12" (wS_VISIBLE .|. wS_CHILD .|. wS_TABSTOP) 0 (Just 220) (Just 10) (Just 20) (Just 20) dialogCtrl Nothing hdl
	return (insetCtrl, dialogCtrl)
...
main = do
	...
	frameWindow "Hoss" Nothing Nothing $ \dialogs wnd msg wParam lParam -> if msg == wM_CREATE then do
			(insetCtrl, dialogCtrl) <- initControls wnd
			...
			writeIORef text $ renumber 0 $ if caret == -1 then
				...
				if wParam == vK_LEFT then
					...
					textVal
					(\ch -> let (caret', ls) = if onBoundary caret textVal then
								(caret + 1, doInsert caret textVal)
							else
								(caret, textVal) in
						fromView ls (take caret' (toView ls) ++ first (\x -> x { string = string x ++ [ch] }) (toView ls !! caret') : drop (caret' + 1) (toView ls)))
